...
function initScroll(sel, ass, axl) {
	if('function' == typeof IScroll) {
		var scrollEle = document.querySelectorAll(sel),sl = scrollEle.length;
		if(sl) {
			var kr = axl ? false : true,
				op = {
					scrollX: kr,
					scrollY: !kr,
				...
				sop = ass ? Object.assign(op, ass) : op,
				sr = Array.prototype.map.call(scrollEle,function(n){
						return new IScroll(n, sop);
					});
			return sr.length == 1 ? sr[0] : sr;
		} else {
			return null;
		}
	} else {
...
function initSwiper(sel, ass, axl) {
	if('function' == typeof Swiper) {
		var swiperEle = document.querySelectorAll(sel),sl = swiperEle.length;
		if(swiperEle.length) {
			var op = { direction : axl ? 'vertical' : 'horizontal' },
				sop = ass ? Object.assign(op, ass) : op;
			sr = Array.prototype.map.call(swiperEle,function(n){
					return new Swiper(n, sop);
				});
			return sr.length == 1 ? sr[0] : sr;
		} else {
			return null;
